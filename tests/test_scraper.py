...
# Import the scraper and your models
...
async def test_fetch_page_success(scraper):
    ...
    mock_client.get.assert_called_once_with("https://www.shl.com/test", headers=scraper.headers)
...
async def test_fetch_page_retries_on_500(scraper):
    ...
    # Temporarily remove wait times so the test runs instantly instead of waiting exponentially
    with patch("tenacity.nap.time.sleep", return_value=None):
        with pytest.raises(Exception):
            await scraper.fetch_page(mock_client, "https://www.shl.com/fail")
    
    # Verify that it attempted 3 times before giving up
...
async def test_parse_catalog_page_extracts_individual_tests(scraper):
    ...
    <div>
        ...
        <a class="product-link" href="/solutions/products/verify-numerical/">Verify - Numerical Reasoning</a>
        <a class="product-link" href="/solutions/products/opq32r/">OPQ32r</a>
    </div>
    """
    # Note: Ensure your scrape_catalog.py matching logic extracts these links
    results = await scraper.parse_catalog_page(mock_html)
    assert len(results) >= 2
    urls = [r["url"] for r in results]
    assert "https://www.shl.com/solutions/products/verify-numerical/" in urls
...
async def test_parse_catalog_page_skips_prepackaged(scraper):
    ...
    <div>
        ...
        <a class="product-link" href="/solutions/products/graduate-pack/">Graduate Pack</a>
    </div>
    <div>
        <h2>Individual Test Solutions</h2>
        <a class="product-link" href="/solutions/products/opq32r/">OPQ32r</a>
...
async def test_parse_assessment_page_extracts_description(scraper):
    mock_html = """
    <html>
        <head><meta name="description" content="This is a secure personality evaluation tool."></head>
        <body><main>Main content backup description text.</main></body>
    </html>
    """
    res = await scraper.parse_assessment_page(mock_html, "https://www.shl.com/opq", "OPQ32r")
...
async def test_parse_assessment_page_detects_remote_testing(scraper):
    mock_html = "<html><body>This assessment supports remote proctoring and online testing.</body></html>"
    res = await scraper.parse_assessment_page(mock_html, "https://www.shl.com/test", "Test")
...
async def test_parse_assessment_page_detects_adaptive(scraper):
    mock_html = "<html><body>Features our computer adaptive testing framework.</body></html>"
    res = await scraper.parse_assessment_page(mock_html, "https://www.shl.com/test", "Test")
...
async def test_parse_assessment_page_extracts_duration(scraper):
    mock_html = "<html><body>The test has a time configuration of 45 minutes for completion.</body></html>"
    res = await scraper.parse_assessment_page(mock_html, "https://www.shl.com/test", "Test")
...
async def test_infer_test_type_personality(scraper):
    mock_html = "<html><body>Description of an OPQ occupational questionnaire.</body></html>"
    res = await scraper.parse_assessment_page(mock_html, "https://www.shl.com/opq", "OPQ Personality Test")
    assert res["test_type"] == "P"

# 10. test_infer_test_type_ability
@pytest.mark.asyncio
async def test_infer_test_type_ability(scraper):
    mock_html = "<html><body>Measures numerical and inductive reasoning ability.</body></html>"
    res = await scraper.parse_assessment_page(mock_html, "https://www.shl.com/num", "Numerical Reasoning")
    assert res["test_type"] == "A"

# 11. test_infer_test_type_knowledge
@pytest.mark.asyncio
async def test_infer_test_type_knowledge(scraper):
    mock_html = "<html><body>A technical evaluation test covering modern java and python rules.</body></html>"
    res = await scraper.parse_assessment_page(mock_html, "https://www.shl.com/code", "Coding Knowledge")
    assert res["test_type"] == "K"

# 12. test_infer_test_type_default
@pytest.mark.asyncio
async def test_infer_test_type_default(scraper):
    mock_html = "<html><body>Random context data with zero standard categorizations.</body></html>"
    res = await scraper.parse_assessment_page(mock_html, "https://www.shl.com/rand", "Mystery Product")
    assert res["test_type"] == "A"
